...
export function UrlForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message = "Request failed before the server returned a response.";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <label className="block space-y-2">
        <span className="text-sm font-medium text-zinc-700">Article URL</span>
        <input
          type="url"
          required
          value={url}
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {isSubmitting ? "Extracting..." : "Extract article"}
        </button>
        <span className="text-xs text-zinc-500">
          Supports standard article and blog pages.
        </span>
      </div>
